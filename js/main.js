...
(function () {
  ...
  if (burger && mmenu) {
    ...
    var burgerMq = window.matchMedia('(max-width: 940px)');
  ...
  if (mbar) {
    ...
    function onScrollBar() {
      ...
      // guard the lookup — pages like About/Works have no #contact section,
      // and an unguarded null deref here would abort the rest of this IIFE
      // (FX pack, portfolio filter) on those pages.
  ...
  /* ---------- FX pack (only pages with <body class="fx">) ---------- */
  if (document.body.classList.contains('fx')) {
    var finePointer = window.matchMedia('(hover: hover) and (pointer: fine)').matches;

    /* cursor spotlight: a soft teal glow follows the pointer over dark sections */
    if (finePointer) {
      document.querySelectorAll('.section--dark').forEach(function (sec) {
        sec.addEventListener('mousemove', function (e) {
          var r = sec.getBoundingClientRect();
          sec.style.setProperty('--fx-x', ((e.clientX - r.left) / r.width * 100).toFixed(1) + '%');
          sec.style.setProperty('--fx-y', ((e.clientY - r.top) / r.height * 100).toFixed(1) + '%');
        });
        sec.addEventListener('mouseenter', function () { sec.classList.add('fx-lit'); });
        sec.addEventListener('mouseleave', function () { sec.classList.remove('fx-lit'); });
      });
    }
  }

  /* ---------- works portfolio filter (progressive enhancement) ---------- */
  var pgrid = document.getElementById('pgrid');
  if (pgrid) {
    var pbtns = document.querySelectorAll('.pfilter__btn');
    var pcards = pgrid.querySelectorAll('.pcard');
    var pempty = document.querySelector('.pgrid__empty');
